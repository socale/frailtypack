...
server <- function(input, output, session) {
  ...
  #                                     formula.terminalEvent=~v1,data=mulData(),n.knots=c(8,8,8),
  #                                     kappa=c(1,1,1),initialize=FALSE, hazard = "Splines")
  # )
  
  # modMul <- eventReactive(input$goButton_mul, {
  #                         mulData <- mulData()
  #                         multivPenal(data=mulData,as.formula(paste("Surv(",paste(input$time_mul, collapse = " , "),",",paste(input$cens_ind_mul),")~ cluster(",paste(input$group_mul),") + event2(",paste(input$event2_mul),") + ",paste(input$co_var_mul_rec, collapse = "+"),
  #                                     "+ terminal(",paste(input$terEvent_mul),")")),as.formula(paste("formula.Event2=~",paste(input$co_var_mul_rec2, collapse = "+"))),as.formula(paste("formula.terminalEvent=~",paste(input$co_var_mul_ter, collapse = "+"))),
  #                                     n.knots=c(8,8,8),kappa=c(1,1,1),initialize=FALSE,hazard = "Splines")
  # })
  ...
  modSurr <- eventReactive(input$goButton_surr,{
    ...
    #                 nb.iterPGH = as.numeric(input$nb.iterPGH_surr), sigma.ss.init = as.numeric(input$sigma.ss_surr), sigma.tt.init = as.numeric(input$sigma.tt.init_surr), sigma.st.init = as.numeric(input$sigma.st.init_surr), 
  ...
  datapredsurr2 <- reactive({
    if (nrow(datapredsurr()>0)) {hot_to_rJ(input$tab_pred_surr)}
    else {NULL}
  })
  ...
  predSurr <- eventReactive(input$goButton_predSurr,{
    ...
    #datapredsurr2 <- datapredsurr2()
    #predict(modSurr(), datapred = datapredsurr2, var.used = input$var_used_surr)
    #predict(modSurr(), datapred = NULL, var.used = input$var_used_surr)
  })
  ...
  # output$left_cens_val_trivnl <- renderUI({
  #   if (input$left_cens_trivnl=='yes'){
  #     tagList(
  #       numericInput("left_cens_val_trivnl", h5(":"), -3.33)
  #     )}
  # })
  ...
  output$strdatapredcox1 <- renderPrint({
    str(input$tab_pred_cox)
  })
  ...
  datamul1 <- eventReactive(input$goButton_mul,{
    noms <- c("Alors hein",names(mulData()))
    noms
  })
  
  datamul2 <- eventReactive(input$goButton_mul,{
    mulData <- mulData()
    mod <-  multivPenal(Surv(TIMEGAP,INDICREC)~cluster(PATIENT)+v1+v2+
                                                   event2(INDICMETA)+terminal(INDICDEATH),formula.Event2=~v1+v2+v3,
                                                 formula.terminalEvent=~v1,data=mulData,n.knots=c(8,8,8),
                                                 kappa=c(1,1,1),initialize=FALSE, hazard = "Splines")
    # mod <- frailtyPenal(Surv(TIMEGAP,INDICREC)~v1+v2, data = mulData(), n.knots = 8, kappa = 10000,
    #                     hazard = "Splines")
    # mod <- additivePenal(as.formula(paste("Surv(",paste(input$time_mul, collapse = " , "),",get(input$cens_ind_mul))~cluster(get(input$group_mul))+
    #                      ", paste(input$co_var_mul_rec, collapse = " + "),"+ slope(",input$co_var_mul_ter,")")),
    #                      data=mulData(),n.knots=8,kappa= 10000)
    # mod <-  multivPenal(Surv(gaptime,event)~cluster(id)+ncc.wts+
    #                                                event2(cov1)+terminal(cov2),formula.Event2=~ncc.wts,
    #                                              formula.terminalEvent=~ncc.wts,data=mulData(),n.knots=c(8,8,8),
    #                                              kappa=c(1,1,1),initialize=FALSE, hazard = "Splines")    
    mod 
    # Voyons voir si le pb vient de multivPenal ou de mulData() DANS multivPenal
    ## LE PROBLEME NE SE PRODUIT QUE AVEC MULTIVPENAL ET MULDATA() ?!
    # muldata() ET frailtyPenal marchent
    # mulData() ET additivePenal marchent (meme avec les inputs) (bon ça converge pas mais le calcul se lance)
    # dataMultiv ET multivPenal marchent
    # mulData() ET multivPenal ne marchent pas...
    # dataNCC et multivPenal plantent R (mais je pense que ça marche)
    # dataNCC en tant que mulData() ET multivPenal ne marchent pas
  })
  ...
  output$modmul1 <- renderPrint(datamul1())
  output$modmul2 <- renderPrint(datamul3())
  #output$modmul2 <- renderTable(datamul2())
  # output$modmul1 <- renderTable(datamul1(),rownames = TRUE)
  # output$modmul2 <- renderTable(datamul2())
  # output$modmul3 <- renderTable(datamul3())
  # output$modmul4 <- renderTable(datamul4())
