...
!    !print*,"funcSurrNN_MC ligne 47: je vaut",funcSurrNN_MC
    ...
    ! i = position du cluster sur lequel on se trouve
     use var_surrogate
    use comon, only: ve,vedc,alpha !eta,theta

    IMPLICIT NONE
    integer ::n !j,k 
    integer,intent(in):: i
    !integer,intent(in)::n
    double precision:: c2,c3,c4,vs !c5,C1,s1,vt
...
!    !print*,"funcSurrNN_MC ligne 47: je vaut",funcSurrNN_MC
    ...
    ! i = position du cluster sur lequel on se trouve
     use var_surrogate
    use comon, only: eta !ve,vedc,theta,alpha

    IMPLICIT NONE
    integer ::n_i !j,k,dimInt 
    integer,intent(in):: i
    !integer,intent(in)::n
    double precision:: C1,c2,c3,c4 !s1,c5,vs,vt
...
!    !print*,"funcSurrNN_MC ligne 47: je vaut",funcSurrNN_MC
    ...
    IMPLICIT NONE
    integer ::n_i !dimInt,j,k 
    integer,intent(in):: i
    !integer,intent(in)::n
    double precision:: c1,c3,c4 !s1,vs,vt
...
!    !print*,"funcSurrNN_MC ligne 47: je vaut",funcSurrNN_MC
 ...
 double precision function Integrale_Individuel(vsi,vti,ui,j,npoint1)
    ...
    ! j = individu j du cluster i
    ! npoint = nombre de point de quadrature
     use var_surrogate, only:posind_i,ui_chap,theta2,const_res5,const_res4,&
        deltastar,delta,adaptative,xx1,ww1,invBi_chol_Individuel,alpha_ui,frailt_base,switch_adaptative
        !invBi_chol_Essai,invBi_cholDet_Essai,essai_courant,estim_wij_chap,nparamfrail,&
        !ntrials,nsujeti,vs_i,vt_i,individu_j,ui_chap_Essai
...
!    use func_adaptative
    ...
    double precision::ss1,auxfunca,ss,wsij,wtij !ss2,test,rho
...
!    call cpu_time(c3)
    ...
    ! 2 effet aleatoire correlee au niveau individeul +2 effet aleatoire correles en interaction avec le traitement + 2 effets aleatoires risque de base:
     ! integrale au niveau individuel
    double precision function Integrale_Individuel_MC_cor(vsi,vti,ui,uti,j,nsimu,ndim,frailij)
        ! vsi= frailtie niveau essai associe a s
        ! vti= frailtie niveau essai associe a t
        ! j = individu j du cluster i
        ! nsimu = nombre de boucle MC
        ...
        use var_surrogate, only: delta,deltastar,const_res4,const_res5,frailt_base,posind_i !Vect_sim_MC
    ...
    ! i = position du cluster sur lequel on se trouve
     use var_surrogate
    use comon, only: ve,vedc,eta,alpha !theta

    IMPLICIT NONE
    integer ::n,dimInt !j,k
    integer,intent(in):: i
    !integer,intent(in)::n
    double precision:: C1,c2,c3,c4,vs !vt,c5,s1
    double precision,intent(in),dimension(:)::frail
...
!    !print*,"funcSurrNN_MC ligne 47: je vaut",funcSurrNN_MC
    ...
    use var_surrogate
    use comon, only: ve,eta !theta,vedc
...
recursive function gaussHermMultMC(frail1,frail,i,k,x,w,inc) result(herm)
 ...
 double precision function funcSurrNN_Essai_2t(frail,i)
    ...
    integer ::n !j,k
    integer,intent(in):: i
    double precision:: C1,c2,c3,c4,vs,vt !c5
 ...
 double precision function funcSurrNN_Essai_t(frail,i)
    ...
    use var_surrogate
    use comon, only: ve,vedc,alpha !eta,theta,res1,aux1

    IMPLICIT NONE
    integer ::n !j,k
    integer,intent(in):: i
    double precision:: C1,c2,c3,c4,vs !vt,c5,m,s1
    ...
    !use comon,only:nig,auxig,alpha,theta,res1,res3,aux1,cdc
    use var_surrogate
    use comon, only: alpha !ve,vedc,eta,theta,res1,aux1

    IMPLICIT NONE
    !integer ::j !n=taille du tableau frail
    integer,intent(in):: i
    double precision:: C1,c2,c3,c4,vs !c5,m,s1,vt
    double precision,intent(in)::frail
    !double precision, dimension(1,2)::m1,m3
