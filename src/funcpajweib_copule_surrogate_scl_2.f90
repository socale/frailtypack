...
!========================          FUNCPAJ_SPLINES         ====================
    double precision function funcpajweib_copule_surrogate(b,np,id,thi,jd,thj,k0)
...
!ccccccccccccccccccccccccccccccccccccccccc
    ...
    res2s_sujet(i)=rho_S * gamma_S * ((t1(i))**(gamma_S - 1.d0))  * dexp(vet) ! baseline hazard for subject i
        ...
        ! if ((res2s(pourtrial(i)).ne.res2s(pourtrial(i))).or.(abs(res2s(pourtrial(i))).ge. 1.d30)) then
            ! funcpajweib_copule_surrogate=-1.d9
            ! goto 123
        ! end if    
        
        const_res4(g(i)) = const_res4(g(i)) + (rho_S * t1(i)**gamma_S)* dexp(vet) ! cumulative baseline hazard for subject i
...
!ccccccccccccccccccccccccccccccccccccccccc 
    ...
    do k=1,ng  
        ...
        res2_dcs_sujet(k)=rho_T * gamma_T * ((t1dc(k))**(gamma_T - 1.d0)) * dexp(vet2)
...
!**************INTEGRALES ****************************
    ...
    if ((res.ne.res).or.(abs(res).ge. 1.d30).or.(res .ge. 0.d0)) then
        ...
        funcpajweib_copule_surrogate = res 
...
123     continue
    ...
    end function funcpajweib_copule_surrogate
