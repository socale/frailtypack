...
!========================          FUNCPAJ_SPLINES         ====================
    double precision function funcpajweib_surrogate(b,np,id,thi,jd,thj,k0)
...
! *** NOUVELLLE DECLARATION F90 :
    ...
    double precision::pe1,pe2,inv,som1,som2,res,vet,vet2,h1,varS1,varT1,covST1,som_cont_0, rho_S, rho_T, gamma_S, gamma_T
...
!    !print*,'debut funcpa'
    ...
    gamma_S = dexp(bh(1)) ! shape S
	rho_S =  dexp(bh(2)) !scale_S
	gamma_T = dexp(bh(3)) ! shape T
	rho_T = dexp(bh(4))   ! scale T
	
	! gamma_S = bh(1)**2.d0 ! shape S
	! rho_S =  bh(2)**2.d0 !scale_S
	! gamma_T = bh(3)**2.d0 ! shape T
	! rho_T = bh(4)**2.d0  ! scale T
	
	
	!variable de module utilisees dans le joint
	betaD = gamma_S! shape S
	etaD = rho_S!scale S
	betaT = gamma_T! shape T
	etaT = rho_T! scale T
...
!                  !write(*,*)'*** funcpaj_splines vet',vet,ve(i,j),i,j
	...
	! Pour le critère de substitution
	res2s_sujet(i)=dlog(rho_S) + dlog(gamma_S) + (gamma_S - 1.d0) * dlog(t1(i)) + vet
         
        if((c(i).eq.1))then
            res2s(pourtrial(i)) = res2s(pourtrial(i))+res2s_sujet(i)
        endif  
        if ((res2s(pourtrial(i)).ne.res2s(pourtrial(i))).or.(abs(res2s(pourtrial(i))).ge. 1.d30)) then
            funcpajweib_surrogate=-1.d9
            goto 123
        end if    
        ...
        const_res4(g(i)) = const_res4(g(i))+(rho_S * t1(i)**gamma_S)* dexp(vet)
...
!ccccccccccccccccccccccccccccccccccccccccc 
    ...
    do k=1,ng  
        ...
        res2_dcs_sujet(k) = dlog(rho_T) + dlog(gamma_T) + (gamma_T - 1.d0) * dlog(t1dc(k)) + vet2
        
        if(cdc(k).eq.1)then
            !!print*,"k=",k,"size(nt1dc)",size(nt1dc),"nt1dc(k)=",nt1dc(k),"dut2(nt1dc(k))=",dut2(nt1dc(k))
            !!print*,"dut2(1:10)",dut2(1:10)
            res2_dcs(pourtrial(k)) = res2_dcs(pourtrial(k)) + res2_dcs_sujet(k)
        ...
        !    goto 123
        !end if
        const_res5(g(k)) = const_res5(g(k))+ (rho_T * t1dc(k)**gamma_T) * dexp(vet2) 
...
!            !print*,"funcpajweib_surrogate_scl.f90 lige 307: quadrature pas encore implémenté"
...
123     continue
    ...
    end function funcpajweib_surrogate
