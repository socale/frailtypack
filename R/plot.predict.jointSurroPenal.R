...
#' of the character strings \code{"blank", "solid", "dashed", "dotted", "dotdash", "longdash",
...
plot.predict.jointSurroPenal= function(object, from = -2, to = 2, type = "Coef", var.used = "error.estim", 
  ...
  if(type == "HR"){ # log HR
    ...
    points(ste(object, var.used = var.used),1)
  }
  else{
    abline(h = 0, col = "cyan", lty = 4)
    points(ste(object, var.used = var.used),0)
  }
  abline(v = ste(object, var.used = var.used), col = "cyan", lty = 4)
}