...
if(is.null(x$family)){ 
  ...
  if (x$istop == 1){
    if (!is.null(coef)){
      ...
      if (x$nvarnotdep[1] == 0){
        ...
        if (x$noVar1 == 0){
          ...
          prmatrix(tmp[1:x$nvarnotdep[1], -2 ,drop=FALSE])
          if(x$global_chisq.test==1){
            cat("\n")
            prmatrix(tmpwald)
          }
          ...
          cat("Shape for the Weibull hazard function is :", round(x$shape.weib[1],2), "\n")
            ...
            cat("Scale for the log-logistic hazard function is :", round(x$scale.weib[1],2), "\n")	
            cat("Shape for the log-logistic hazard function is :", round(x$shape.weib[1],2), "\n")
          }
          else if (x$family[2]==2){
            cat("Scale for the log-normal hazard function is :", round(x$scale.weib[1],2), "\n")	
            cat("Shape for the log-normal hazard function is :", round(x$shape.weib[1],2), "\n")
          }
          else if (x$family[2]==3){
            cat("Scale for the Weibull hazard function is :", round(x$scale.weib[1],2), "\n")	
            cat("Shape for the Weibull hazard function is :", round(x$shape.weib[1],2), "\n")
      ...
      if (x$nvarnotdep[2] == 0){
        ...
        if (x$noVar2 == 0){
          ...
          prmatrix(tmp[-c(1:x$nvarnotdep[1]), -2 ,drop=FALSE])
          if(x$global_chisq.test_d==1){
            cat("\n")
            prmatrix(tmpwalddc)
          }
          cat("\n")
          if (x$family[1]==0){
            cat("Scale for the Weibull hazard function is :", round(x$scale.weib[2],2), "\n")	
            cat("Shape for the Weibull hazard function is :", round(x$shape.weib[2],2), "\n")
          }
          else if (x$family[1]==1){
            cat("Scale for the log-logistic hazard function is :", round(x$scale.weib[2],2), "\n")	
            cat("Shape for the log-logistic hazard function is :", round(x$shape.weib[2],2), "\n")
          }
          else if (x$family[1]==2){
            cat("Scale for the log-normal hazard function is :", round(x$scale.weib[2],2), "\n")	
            cat("Shape for the log-normal hazard function is :", round(x$shape.weib[2],2), "\n")
          }
          else if (x$family[1]==3){
            cat("Scale for the Weibull hazard function is :", round(x$scale.weib[2],2), "\n")	
            cat("Shape for the Weibull hazard function is :", round(x$shape.weib[2],2), "\n")
    ...
    if (x$logNormal == 0){
      if (indic_alpha == 1 & x$joint.clust<=1){
        cat("   theta (variance of Frailties, w):", frail, "(SE (H):",seH.frail, ")", "p =", ifelse(signif(1 - pnorm(frail/seH.frail), digits - 1) == 0, "< 1e-16", signif(1 - pnorm(frail/seH.frail), digits - 1)), "\n")
        cat("   gamma (w^gamma for terminal event):", x$alpha, "(SE (H):",sqrt(diag(x$varH))[2], ")", "p =", ifelse(signif(1 - pchisq((x$alpha/sqrt(diag(x$varH))[2])^2,1), digits - 1) == 0, "< 1e-16", signif(1 - pchisq((x$alpha/sqrt(diag(x$varH))[2])^2,1), digits - 1)), "\n")
      }else if (x$joint.clust ==2) {
        cat("   theta (variance of u, association between recurrences and terminal event):", frail, "(SE (H):",seH.frail, ")", "p =", ifelse(signif(1 - pnorm(frail/seH.frail), digits - 1) == 0, "< 1e-16", signif(1 - pnorm(frail/seH.frail), digits - 1)), "\n")
        cat("   eta (variance of v, intra-subject correlation):", x$eta, "(SE (H):",sqrt(((2 * (x$eta^0.5))^2) * diag(x$varH)[2]), ")", "p =", ifelse(signif(1 - pnorm (x$eta/sqrt(((2 * (x$eta^0.5))^2) * diag(x$varH)[2]),1), digits - 1) == 0, "< 1e-16", signif(1 - pnorm (x$eta/sqrt(((2 * (x$eta^0.5))^2) * diag(x$varH)[2]),1), digits - 1)), "\n")
      } else {
